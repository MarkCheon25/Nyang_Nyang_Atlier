...
- **상태 소유자 = ⑥ 작업 관리 — 확정 (2026-07-27).** 일시정지는 "어디까지 그렸는가"(스트로크 인덱스·계획 집합·펜 상태)를 보존·재개하므로 그 소유 모듈이 정해져야 했다. **`src/operator` 의 C++ 노드(`job_manager`)가 소유한다** — ③ 로봇제어는 궤적을 실행하고 진행 사실을 ⑥에 올려보낼 뿐이다. 상태 전이 전체는 `src/operator/README.md`.
