...
> ⚠️ **SAM3 는 이 PC 에서 돌릴 수 없다 — 편의가 아니라 제약이다.** 설치까지 마친 뒤
> 막혔다. 근거는 §7. 따라서 아래 결과는 **"SAM3 를 아직 안 붙인 상태"가 아니라
> "우리가 붙일 수 있는 최대치"** 다.
...
### ✅ 2026-08-16 재실행 — **우회 없이, 실물 크기로 5/5**

팀원 실측 자산(`markch/hcr5_ros2`)으로 전환하고 **우리 우회를 전부 걷어낸 뒤** 같은
경로를 다시 돌렸다. 백엔드도 mock 이 아니라 **MuJoCo** 다.

```
입력    비전 마스크 5 개 → contour_pixel(비전팀 실제 코드) → 107 점
크기    126.4 × 150.0 mm     긴 변 150.0 mm 정확 (draw_size 0.15 m)
실행    5/5 · 모든 fraction 1.00        ← 우회 없음
시간    계획 35.78 s / 실측 36.01 s  →  오차 +0.6 %
```

| | 8-16 1 차 (mock, 우회 有) | 8-16 2 차 (MuJoCo, 우회 無) |
|---|---|---|
| 그림 크기 | 67.4 × 80.0 mm | **126.4 × 150.0 mm** |
| 실행 | 5/5 (우회 필요) | **5/5 (우회 불필요)** |
| 추종 오차 | — | **+0.6 %** |

**이것이 전 구간이 우회 없이 도는 첫 실행이다.** 비전 마스크에서 MuJoCo 물리까지.
...
| 합성 데이터, 최적화 후 | 4.8 % |
| **실제 비전 데이터, 5/5** | **3.0 %** |
| **실측 자산 전환 후, 15 cm, 5/5** | **3.0 %** ← 재확인 |

**2026-08-16 재실행에서도 정확히 3.0 % 다.** 그림을 8 cm → 15 cm 로 키우고 속도 한계를
실측값으로 낮췄는데도 비율이 그대로다. F3.1 은 이번에 거리를 **44.1 % 줄였지만**
(430.1 → 240.5 종이mm) 시간 기여는 여전히 3 % 미만이다.

**시간을 지배하는 것은 `cat` 외곽선 하나다** — 54 점짜리가 **24.77 s** 로 전체 35.78 s 의
**69 %** 를 차지한다. 나머지 4 개를 다 합쳐도 10 s 다.
...
| | 무엇 | 담당 | 비고 |
|---|---|---|---|
| ① | `SAM3 → contour_pixel` 한 고리 | **비전팀** | 우리 PC 에서 불가(§7). 하류는 전부 검증됐으므로 남은 것은 이 고리뿐이다 |
| ② | 마스크 품질 변동 | **비전팀** | 재생으로 못 덮는 유일한 항목 — 구멍·여러 덩어리일 때 `contour_pixel` 이 "가장 큰 것 하나" 만 쓰는 경로. ① 과 같은 이유로 여기서 확인 불가 |
| ③ | mock 용 런치 파일 | moveit2 | 관절 한계 우회를 포함한 것을 `drawing_cat` 에 두면 매번 공유 설정을 건드리지 않아도 된다 |
| ④ | AC2 재논의 | 팀 전체 | 위 §4 수치를 팀에 가져갈 것 |
| ⑤ | URDF 관절 한계 수정 | 시뮬레이션팀 | **mock 재현으로 근거가 강화됐다** |

---

## 7. 🔴 SAM3 는 이 PC 에서 돌릴 수 없다 (2026-08-16 확인)

대역을 쓴 것이 **게을러서가 아니라는 근거**다. 비전팀 절차대로 설치를 끝까지 마친 뒤
실행 단계에서 막혔고, 원인 셋 중 둘은 이 PC 에서 고칠 수 없다.

| | 막는 것 | 실측 |
|---|---|---|
| ① | **GPU 커널이 없다** | 설치된 torch 지원 = `sm_75` 이상 / GTX 1050 = **`sm_61`**(Pascal). CUDA 13 이 이 세대를 잘라냈다 |
| ② | **VRAM 이 모델보다 작다** | `sam3.pt` **3.3 GB** vs VRAM **2.0 GB** |
| ③ | **RAM 이 모자란다** | 총 7.5 GB 중 가용 4.2 GB. CPU 추론이 스왑으로 넘어가 **멈춘 것처럼 보인다**(실제로 2 회 발생) |

```
torch 2.13.0+cu130  arch list : ['sm_75','sm_80','sm_86','sm_90','sm_100','sm_120']
nvidia-smi          GTX 1050  : compute_cap 6.1,  2048 MiB
free -h                       : 7.5Gi 총 / 4.2Gi 가용
```

**①은 GPU 를 컨테이너에 넘겨줘도 해결되지 않는다** — 실행할 커널 자체가 없어 CPU 로
떨어진다. 이것이 CPU 폴백의 진짜 원인이었다(드라이버·컨테이너 설정 문제가 아니다).
`src/vision/compose.yml:32` 에 GPU 통과 설정이 없는 것은 사실이지만, 그것을 고쳐도
①②가 남는다.

**②는 ①의 우회를 막는다.** 구버전 torch(`sm_61` 포함)로 낮춰도 3.3 GB 모델이 2.0 GB
VRAM 에 들어가지 않는다. fp16 으로 반을 줄여도 가중치만 ~1.65 GB 라 활성값 자리가 없다.

> **비전팀에 전할 것** — moveit2 개발 PC 사양(`sm_61` / VRAM 2 GB / RAM 7.5 GB)에서는
> SAM3 실행이 불가능하다. 배포 대상 환경의 최소 사양을 정할 때 참고가 된다.
