...
**작업일** 2026-08-08 · **브랜치** `ysh_moveit_ros2_control` · **담당** ysh (moveit2 모듈)
...
| 펜 방향 RViz 육안 확인 | ⬜ **미검증** — FK 계산으로는 확인됨(§5.2) |
...
1단계  description + moveit_config + mock 실행                🔶 거의 완료 (§0 의 ⬜ 2건)
 │
2단계  2-1~2-3 궤적 시각화  ───  2-4 역방향 검증(선→궤적)     ⬜
 ║          (관찰 방향)              (그리기 가능성 판정)
 ╠═ 병렬 ═ 3-1 가짜 로봇 스텁 · 3-2 상태 브릿지 · 3-3 구현     ⬜
 ║
[토/일 인천] movej 프로토콜 캡처 + 연속성 관찰                ⬜ ← 팀 요청 필요
 │
3-4   명령 브릿지 (궤적 → movej)                              ⬜ (프로토콜 대기)
 │
4단계  실기 환경 구성 + 제어                                   ⬜
 │
...
| joint_2 | ±90° | 유지 | 실기는 ±360°지만 넓힐 실익 없음 |
...
### 10.1 🔥 인천 출장자께 — 캡처 요청 (시간 급함)

공유받은 현황에 *"로봇암 기능테스트 중 (주요함수 **movej, movel**, base좌표계, 엔드이펙터 좌표계, ...)"*
가 있다. **이것이 우리 3-4 단계의 블로커와 정확히 같은 대상이다.**

펜던트로 movej/movel 을 조작하는 **동안 `capture.py` 를 같이 돌리기만 하면** 프로토콜이 잡힌다.
조작자 부담은 거의 없다.

```bash
# 준비: 노트북 유선 NIC 를 컨트롤러에 직결
...
# 캡처 (읽기 전용 — 로봇에 아무것도 쓰지 않음)
mkdir -p ~/cap_movej
python3 src/drivers/hcr_comm/tools/capture.py 192.168.0.20 1883 ~/cap_movej
# "[READY]" 뜰 때까지 6초 대기 (이때 로봇 건드리지 말 것)
```

`[READY]` 이후 **사이에 3초씩 쉬면서** 아래를 하나씩:

| 순서 | 조작 | 얻는 것 |
|---|---|---|
| 1 | movej 로 관절자세 이동 | **절대 관절이동 명령** ← 최우선 |
| 2 | movel 로 직선 이동 | 직선이동 (그리기에 직결) |
| 3 | 속도/가속 바꿔 movej 재실행 | 속도·가속 인자 이름 |
| 4 | 웨이포인트 티치 → 프로그램 재생 | 프로그램 실행 경로 |
| 5 | base ↔ tool 좌표계 전환 후 이동 | 좌표계 지정 인자 |
| 6 | **movej 를 연속 2~3개 실행** | ★ §10.2 |
| 7 | 각 조작 시점 **펜던트 화면 사진** | 로그 해석에 필수 |

추가로 **가만히 있는 상태 30초 캡처** — `motion/tool/position` · `motion/flange/position` 실측값이
남아, 실기 없이도 §9.2 의 영점·부호 검증을 미리 할 수 있다.

종료는 `Ctrl-C`, `~/cap_movej/novel.jsonl` 만 보내주면 된다.

### 10.2 ★ movej 연속성 — 프로젝트 성패가 걸린 질문

**movej 가 점대점(point-to-point)이면 그리기 선 품질이 결정적으로 나빠진다.**
획 하나를 여러 movej 로 쪼개 보내면 매 구간마다 감속→정지→가속이 생겨 선에 마디가 진다.

> **movej 를 연속으로 2~3개 실행했을 때, 중간에 멈췄다 가는지 / 끊김 없이 이어지는지(blending)**
> — 눈으로 보고 알려주기만 해도 된다. 블렌딩 반경 같은 파라미터가 펜던트에 있는지도 함께.

답이 "매번 정지"면 궤적을 촘촘히 쪼개는 방식이 무의미해지고 **서보 스트리밍 경로를 따로 찾아야 한다.**
3·4단계 설계가 여기서 갈린다. **지금 물으면 공짜, 나중에 알면 재작업이다.**
