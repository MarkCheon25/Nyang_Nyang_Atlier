...
> ## 2026-08-16 — **실물 연동 성공**
>
> 비전 마스크부터 로봇 동작까지 전 구간이 끝에서 끝까지 돌았다. 좌표 변환·축척·
> 메시지·프레임 경계가 전부 실물로 검증됐다. 상세는
> [`Vision Integration Result.md`](Vision%20Integration%20Result.md).
>
> **그 과정에서 AC2 가 사실상 불가능하다는 것이 확정됐다** — 실제 비전 데이터에서
> 펜업 이동은 전체 시간의 **3.0 %** 다 (§2.5 갱신).
...
### 🔴 2026-08-16 갱신 — 실제 비전 데이터에서는 더 나쁘다

실물 연동으로 잰 결과 ([`Vision Integration Result.md`](Vision%20Integration%20Result.md)):

```
펜업 이동   0.14 s    3.0%
그리기      4.57 s   97.0%    ← cat 외곽선 하나가 3.11 s
```
...
F3.1 이 펜업 **거리**를 45.2 % 줄였는데도 **전체 시간 기여는 1 % 남짓**이다.

판정식(펜업 시간 기준)으로는 통과할 수 있지만, **AC2 의 의도가 "전체 작업 시간 단축"
이었다면 순서 최적화로는 달성할 수 없다는 것이 실물로 확정됐다.** 팀이 어느 쪽을
의도했는지부터 정해야 한다.
