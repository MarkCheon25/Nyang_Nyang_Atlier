...
관절      움직인 폭    최대 추종오차
joint_1     4.70°         0.05°
joint_2     7.02°         0.12°      ← 종전 14.96°
joint_3     7.06°         0.18°
joint_4     1.42°         0.07°
joint_5     0.00°         0.00°      ← 안 움직인 관절. 대조군
joint_6     4.70°         0.05°
```

> ⚠️ **"움직인 폭" 을 같이 적는 이유** — 오차값만 보면 정지 상태의 중력 처짐
> (joint_2 0.16° · joint_3 0.17°)과 구분이 안 된다. 실제로 그 둘이 비슷해서 *"캡처가
> 동작 구간을 못 담은 것 아니냐"* 를 따로 확인해야 했다. `joint_2` 가 **7.02° 움직이는
> 동안 오차 0.12°** 이므로 움직인 양의 **1.7 %** 다. `joint_5` 는 움직임도 오차도 정확히
> 0 이라 계측 자체의 대조군이 된다.
>
> 종전 상태가 얼마나 나빴는지도 이 대비로 드러난다 — 7 도쯤 움직이는데 **오차가 14.96°**
> 였다. 오차가 움직임보다 컸다.
