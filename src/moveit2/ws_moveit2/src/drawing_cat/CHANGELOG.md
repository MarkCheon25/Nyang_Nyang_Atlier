...
## 2026-08-16 ⑨ — 비전 실물 연동 관통 · SAM3 는 이 PC 에서 불가로 확정

### 한 것

비전팀 `dy/vision` 실물 코드로 **끝에서 끝까지 관통**했다. 워크트리(`~/Nyang_dy_vision`)로
받아 우리 브랜치에 비전 코드가 섞이지 않게 했다.

```
mask_replay → /vision/parts → contour_pixel → /vision/strokes → draw_cat → mock hardware
 (우리 대역)                   (비전 실제 코드)                    (우리)
```

**`contour_pixel` 부터는 전부 비전팀 실제 코드다.** 검증된 것:

| 항목 | 결과 |
|---|---|
| 메시지 타입 해시 | 실물 DDS 에서 일치 — 미러 방식이 통한다는 실증 |
| 프레임 경계 판정 | 부위 5 개가 **32 ms 버스트**로 도착 → 한 프레임. `frame_timeout_ms: 1500` 충분 |
| bbox 축척 | 444 × 527 px → 긴 변 **80.0 mm** 정확 |
| 종이 → 로봇 축 매핑 | RViz 자취가 **고양이 모양 그대로** |
| 전 스트로크 실행 | **5/5**, `fraction 1.00` (관절 한계 우회 적용 시) |

### SAM3 는 대역을 계속 쓴다 — 편의가 아니라 제약이다

설치를 끝까지 마친 뒤 실행에서 막혔다. 원인 셋 중 둘은 이 PC 에서 고칠 수 없다.

```
torch 2.13.0+cu130  arch list : ['sm_75','sm_80', ...]   ← sm_61 없음
GTX 1050                      : compute_cap 6.1, VRAM 2048 MiB   vs 모델 3.3 GB
free -h                       : 7.5Gi 총 / 4.2Gi 가용  → CPU 추론이 스왑으로
```

GPU 를 컨테이너에 넘겨줘도 **실행할 커널 자체가 없어** CPU 로 떨어진다. 구버전 torch 로
낮춰도 VRAM 에서 다시 막힌다. **→ `mask_replay.py` 는 임시 대역이 아니라 우리 쪽 상시
상류다.** 마스크 품질 확인만 비전팀 몫으로 남는다. 상세는 `docs/Vision Integration Result.md` §7.

### 확인 방법

`docs/Vision Integration Result.md` §5 에 셸 5 개짜리 재현 절차를 남겼다. 같은 문서 §5 의
**"걸렸던 함정 넷"** 도 같이 볼 것 — 특히 `--symlink-install` 은 설정 파일만 즉시 반영하고
**C++ 바이너리는 재빌드해야 한다**.

---

## 2026-08-16 ⑧ — 종이 → 로봇 좌표 변환을 비전팀 구현에 맞췄다

### 무엇이 틀렸나

종이 mm 를 로봇 좌표로 옮기는 축 매핑이 비전팀 `draw_strokes_node.cpp` 와 달랐다.
올바른 규칙은:
...
`flangePose` · `tipPoint` 두 곳에 **동일하게** 적용했다 (한쪽만 고치면 마커와 실제 동작이
어긋난다).

### 왜 여태 안 잡혔나

> ⚠️ **`trace_cli` 로는 원리적으로 못 잡는다.** 그건 **종이 좌표계**에서 비교하므로 로봇
> 쪽 축이 어떻게 배치되든 항상 멀쩡해 보인다. **RViz 자취(로봇 좌표계)로만 드러난다.**

이것이 이번에 처음 검증된 이유다 — 그전까지는 파일 기반 검증만 했다.

### 확인

수치로 1.4e-17 m 일치, 그리고 RViz 자취가 **고양이 모양 그대로** 나왔다 (90° 회전도
거울상도 아님). 사용자 확인: *"고양이 전체 몸, 꼬리, 얼굴 전부 잘 그려졌어"*.
