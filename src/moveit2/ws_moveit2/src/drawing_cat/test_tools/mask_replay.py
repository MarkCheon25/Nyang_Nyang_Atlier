#!/usr/bin/env python3
"""SAM3 없이 `/vision/parts` 를 재생한다 — 비전팀이 올린 마스크 PNG 를 그대로 발행.
...
앞 토막이 무거워서(모델 3.2GB, pip 설치 5~10분) 뒷토막을 시험하려면 매번 그걸 다
갖춰야 한다. 그런데 비전팀이 SAM3 **출력물**(`sam3_output/` 의 마스크 PNG 5장 +
metadata.json)을 같이 올려 두었으므로, 그것을 그대로 발행하면 앞 토막을 대신할 수 있다.

## 나중에 진짜 SAM3 를 붙여도 되는 이유
