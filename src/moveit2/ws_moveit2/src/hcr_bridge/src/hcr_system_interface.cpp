#include "hcr_bridge/hcr_system_interface.hpp"
  ...
  const rclcpp::Time &, const rclcpp::Duration &)
{
  ...
  // 분해능은 병목이 아니므로(hcr_comm README §5.3) 전정밀도 그대로 싣는다.
  {
    std::lock_guard<std::mutex> lk(cmd_mutex_);
...
PLUGINLIB_EXPORT_CLASS(hcr_bridge::HcrSystemInterface, hardware_interface::SystemInterface)
