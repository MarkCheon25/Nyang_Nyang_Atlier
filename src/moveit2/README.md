...
| F4.3 실행·진행 보고 | 궤적 실행 + 현재 스트로크/전체 보고 |
| F4.4 실패 처리 | 실패 스트로크 기록 후 계속 (Phase 1 — 재시도 없음) |

F3이 vision에서 이 모듈로 옮겨진 경위·근거는 `src/vision/README.md`(A. 이미지 → 지도)에 기록.

**진행·완료·실패 사실을 받는 쪽은 `src/operator`(⑥ 작업 관리)다** — 작업 상태("어디까지 그렸는가")를 소유하는 것은 그쪽이고, 이 모듈은 사실을 올려보내고 정지·재개 지시를 받는다 (2026-07-27 확정, `docs/System Architecture.md` §6). 경계 전달 형식은 vision 경계와 함께 미정.
