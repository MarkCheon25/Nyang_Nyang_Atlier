...
// 이 노드가 열어야 할 경계는 **둘**이다 —
//   ① 지도를 moveit2 에 넘기는 것 (A)
//   ② operator 가 요청하는 F1.2 적합성 판정에 답하는 것 (E — 2026-07-27 결정, README 참조)
// 둘 다 moveit2 착수 시점에 함께 정한다. 코어를 ROS 무관 라이브러리로 분리해 둔
// 이유가 그 결정을 늦출 수 있어서다.
//
