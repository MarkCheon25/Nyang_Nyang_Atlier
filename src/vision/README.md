...
- **F1.2 적합성 검사는 하지 않는다** — "통과된 이미지가 들어온다"가 A의 계약 (아래 미결 참조)
...
- **F1.2 적합성 검사의 주체** — Process Flow §2.1 은 운영 레인에 두었으나 "흰 배경
  라인아트냐"는 픽셀을 봐야 판정된다. operator 는 Python(FastAPI)이고 N1 은 전 구간 C++ 이라
  경계가 어긋난다. 현재 A 는 계약 밖으로 두고 진행 중
