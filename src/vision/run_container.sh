...
# GPU 설정은 compose.yml 에 없다 — 필요한 PC 만 이 오버라이드로 제공한다.
# vision 은 CPU 만으로 동작하므로 없는 것이 정상이다 (README §2).
if [ -f "${SCRIPT_DIR}/compose.override.yml" ]; then
    COMPOSE="${COMPOSE} -f ${SCRIPT_DIR}/compose.override.yml"
fi
CONTAINER="vision_dev"

# 컨테이너 사용자를 호스트와 같은 UID/GID로 만들기 위해 compose 에 넘긴다.
# (ws_vision/·data/ 안에 생기는 파일의 소유자가 어긋나지 않게 하기 위함)
export HOST_UID="$(id -u)"
export HOST_GID="$(id -g)"

# 렌더 노드(/dev/dri) 접근 권한은 호스트 GID 기준으로 걸린다. GID 는 PC마다
# 다르므로(video 는 대개 44 로 같지만 render 는 992·993 등 제각각) 호스트에서
# 읽어 compose 의 group_add 로 넘긴다. cv::imshow 를 쓸 때만 관계있다.
