...
> 🔴 **2026-08-16 실기 사용 정지.** 펜 TCP(오프셋 **120.628mm**)가 컨트롤러에 등록되면서
> 웨이포인트의 `tcp`·`flange` 슬롯이 **같은 포즈를 들고 모순**이 됐다 — 최악의 경우 지령에서
> 120mm 어긋난 곳으로 직선 이동한다. `--dry-run` 은 안전하고, **`movej` 는 영향 없다.**
> 판정·해제 조건은 [`movel.md`](movel.md) 머리말 · 업무목록 L28.
