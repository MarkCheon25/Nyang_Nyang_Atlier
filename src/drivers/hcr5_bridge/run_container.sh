#!/usr/bin/env bash
# Nyang_Nyang_Atlier 개발 컨테이너 헬퍼
# 사용: ./run_container.sh {build|up|shell|down|logs|config}
#
# src/moveit2/run_container.sh 의 복제본이며 CONTAINER 만 다르다 (2026-08-16).
# ⚠️ 이 스크립트의 CONTAINER 는 하드코딩이다 — 컨테이너 이름을 바꾸면 여기도 고쳐야
#    shell·logs 가 엉뚱한 컨테이너를 가리키지 않는다.
set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
COMPOSE="docker compose -f ${SCRIPT_DIR}/compose.yml"
# GPU 설정은 compose.yml 에 없다 — PC마다 이 오버라이드로 제공한다 (커밋 대상 아님).
# 없으면 GPU 없이 뜨고 RViz2 3D 가 소프트웨어 렌더링이 된다.
if [ -f "${SCRIPT_DIR}/compose.override.yml" ]; then
    ...
    echo "   GPU를 쓰려면 src/moveit2/README.md §2 의 스니펫으로 이 파일을 만드세요." >&2
    echo "   ⚠️ 서비스 키는 'atlier' 입니다 (moveit2 쪽 스니펫은 'moveit2')." >&2
fi
CONTAINER="nyang_nyang_atlier"

# 컨테이너 사용자를 호스트와 같은 UID/GID로 만들기 위해 compose 에 넘긴다.
# (ws_atlier/ 안에 생기는 파일의 소유자가 어긋나지 않게 하기 위함)
