...
## 🔴 실기 사용 정지 — 펜 TCP 등록으로 전제가 깨졌다 (2026-08-16, 260816-알트도르퍼)

**`movel` 을 실기에 태우지 마라.** `movej` 는 영향 없다(관절값 직접 전송).

무엇이 바뀌었나 — 컨트롤러에 펜 TCP 가 등록됐다: `(−3.675, −1.725, 120.560)`, **오프셋 120.628mm**.
`movel` 은 오프셋이 **0 이던 시절**에 만들어졌고, 그 전제가 코드에 박혀 있다:

```cpp
// movel.cpp  poseTriple()
return Json{{"tcp", pose}, {"flange", pose}, {"joint", joint}};   // ← 같은 pose 를 양쪽에
```

주석은 *"셋이 어긋나면 컨트롤러가 어느 것을 쓰는지 알 수 없으므로 전부 같은 점으로 채운다"* 고 적는다.
그 논리는 **tcp 와 flange 가 실제로 같은 점일 때만** 성립했다. 이제 둘은 120.628mm 떨어져 있고,
웨이포인트는 **물리적으로 불가능한 조합**을 싣는다. 컨트롤러가 어느 슬롯을 채택하는지는 **미확인**이다.

- `plan["coordinates"]["tcp"]` 는 여전히 **영 오프셋 프레임**이고 `attr.frame = "flange"` 다 —
  즉 계획서 자체는 flange 기준이라고 말한다. 그래도 `tcp` 슬롯의 값이 모순인 것은 그대로다
- 최악의 경우 지령 위치에서 **120mm 어긋난 곳**으로 직선 이동한다. 테이블·종이가 그 사거리 안에 있다

**해제 조건** — 아래를 실기에서 확인하고 코드를 고친 뒤에 푼다:

1. 컨트롤러가 웨이포인트의 `tcp` / `flange` 중 **무엇을 쓰는지** 판정 (안전한 자세에서 소진폭 1회)
2. `poseTriple()` 을 둘로 갈라 각 슬롯에 **제 좌표계의 값**을 넣기
3. `--at tcp` / `--at flange` 가 실제로 갈리는지 실증

→ 업무목록 **L28**. 실측 근거 = [`hcr5_comm/measurements/260816_펜TCP/README.md`](../hcr5_comm/measurements/260816_펜TCP/README.md)
