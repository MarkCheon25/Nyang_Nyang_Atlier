...
# ⚠️ `:?` 메시지에 중괄호를 넣지 말 것 — bash 는 **첫 `}` 에서 확장을 끝내고** 남은 `}` 를
# 값에 리터럴로 붙인다. 종전 `${1:?… {w1|w2|home}}` 은 STAGE 를 `w1}` 로 만들어
# 아래 case 가 **어떤 인자로도 전부 불일치**했다 — 이 스크립트는 실행 자체가 불가능했다
# (2026-08-15 03pc 에서 발견·확증. 04pc '미실행' 기록과 일치).
...
BUS=$!
$DEX "$RSRC && python3 src/drivers/hcr5_bridge/tools/ros_trace.py $DUR /tmp/c2_${STAGE}_ros.json" \
